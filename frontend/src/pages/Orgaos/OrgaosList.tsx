...
const vazio: Paginado<Orgao> = { data: [], total: 0, page: 1, pageSize: PAGE_SIZE_PADRAO, totalPages: 1 };
...
export function OrgaosList({ refreshKey, onVisualizar, onEditar, onAlternarStatus }: Props) {
  ...
  const [pageSize, setPageSize] = usePageSize('orgaos');
  ...
  useEffect(() => {
    ...
    setCarregando(true);
    setErro(null);
    listarOrgaos({ filtros, busca: buscaDebounced, orderBy: sort?.campo, orderDir: sort?.direcao, page, pageSize })
  ...
  function renderCell(key: string, o: Orgao): ReactNode {
    switch (key) {
      case 'acoes':
        return (
          <AcoesGrade recurso="CONFIG_ORGAOS">
            <IconBtn title="Visualizar" onClick={() => onVisualizar(o)}><Eye className="h-4 w-4" /></IconBtn>
            <IconBtn exige="EDICAO" title="Editar" onClick={() => onEditar(o)}><Pencil className="h-4 w-4" /></IconBtn>
            <IconBtn exige="EDICAO" title={o.ativo ? 'Inativar' : 'Reativar'} danger={o.ativo} onClick={() => onAlternarStatus(o)}><Power className="h-4 w-4" /></IconBtn>
          </AcoesGrade>
        );
      case 'status':
