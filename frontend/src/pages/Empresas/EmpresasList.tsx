...
const vazio: Paginado<Empresa> = { data: [], total: 0, page: 1, pageSize: PAGE_SIZE_PADRAO, totalPages: 1 };
...
export function EmpresasList({ refreshKey, onVisualizar, onEditar, onAlternarStatus }: Props) {
  ...
  const [pageSize, setPageSize] = usePageSize('empresas');
        ...
        <table className="w-full text-left text-[13px]" style={{ tableLayout: 'fixed', minWidth: totalWidth }}>
          ...
          <tbody className="divide-y divide-ink-100 dark:divide-ink-800">
            {carregando ? (
              ...
              <tr>
                <td colSpan={colunas.length} className="py-16 text-center">
                  <p className="text-sm text-ink-500 dark:text-ink-400">Nenhuma empresa encontrada.</p>
                </td>
              </tr>
            ) : (
              data.map((emp) => (
