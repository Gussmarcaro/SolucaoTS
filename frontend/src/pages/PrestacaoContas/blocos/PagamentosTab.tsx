        ...
        <table className="w-full min-w-[640px] text-left text-sm">
          <thead>
            <tr className="border-b border-ink-100 text-xs font-semibold text-ink-500 dark:border-ink-800 dark:text-ink-400">
              ...
              <th className="px-4 py-2.5">Meio</th>
          ...
          <tbody className="divide-y divide-ink-100 dark:divide-ink-800">
            {carregando ? (
              ...
              <tr><td colSpan={5} className="py-10 text-center text-sm text-ink-400">Nenhum pagamento cadastrado.</td></tr>
            ) : (
              lista.map((p) => (
                <tr key={p.id} className="hover:bg-ink-50/70 dark:hover:bg-ink-800/40">
                  <td className="px-4 py-2.5 text-ink-700 dark:text-ink-200">
                    {p.documentoNumero ? <span className="font-mono text-xs">Doc. nº {p.documentoNumero}</span> : <Badge tone="warning">Folha (9999)</Badge>}
                  </td>
                  <td className="px-4 py-2.5 text-ink-600 dark:text-ink-300">{dataBr(p.dataPagamento)}</td>
                  <td className="px-4 py-2.5 text-ink-600 dark:text-ink-300">{p.meioPagamento === 'BANCO' ? 'Banco' : 'Fundo fixo'}</td>
                  <td className="px-4 py-2.5 text-right tabular-nums text-ink-700 dark:text-ink-200">{formatarMoeda(p.valor)}</td>
