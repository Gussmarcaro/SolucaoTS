...
const PAGE_SIZE = 10;
const vazio: Paginado<ServidorCedido> = { data: [], total: 0, page: 1, pageSize: PAGE_SIZE, totalPages: 1 };
        ...
        <table className="w-full text-left text-sm" style={{ tableLayout: 'fixed', minWidth: totalWidth }}>
          ...
          <tbody className="divide-y divide-ink-100 dark:divide-ink-800">
            {carregando ? (
              ...
              <tr>
                <td colSpan={colunas.length} className="py-16 text-center">
                  <UserCog className="mx-auto h-8 w-8 text-ink-300" />
                  <p className="mt-2 text-sm text-ink-500 dark:text-ink-400">Nenhum servidor encontrado.</p>
                </td>
              </tr>
            ) : (
              data.map((s) => (
