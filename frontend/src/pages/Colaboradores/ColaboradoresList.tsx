...
import type { Colaborador, FiltrosColaborador, Paginado } from '@/types/colaborador';

const vazio: Paginado<Colaborador> = { data: [], total: 0, page: 1, pageSize: PAGE_SIZE_PADRAO, totalPages: 1 };

const COLUNAS: ColunaDef[] = [
  { key: 'acoes', label: 'Ações', width: 120, minWidth: 100, align: 'center' },
  { key: 'status', label: 'Status', width: 120, sortKey: 'ativo' },
  { key: 'colaborador', label: 'Colaborador', width: 300, sortKey: 'nome' },
  { key: 'cpf', label: 'CPF', width: 160, sortKey: 'cpf' },
  { key: 'admissao', label: 'Admissão', width: 130, sortKey: 'dataAdmissao' },
  { key: 'salario', label: 'Salário', width: 150, sortKey: 'salarioContratual', align: 'right' },
];
...
export function ColaboradoresList({ refreshKey, onVisualizar, onEditar, onAlternarStatus }: Props) {
  const { usuario: logado } = useAuth();
  const { revelado, alternar } = useDadosPessoais('Colaborador', 'Cadastro de Colaboradores');
  const [pageSize, setPageSize] = usePageSize('colaboradores');
  const [busca, setBusca] = useState('');
  const [status, setStatus] = useState<StatusFiltro>('');
  const [page, setPage] = useState(1);
  const [sort, setSort] = useState<SortState | null>(null);
  const [resultado, setResultado] = useState<Paginado<Colaborador>>(vazio);
  const [carregando, setCarregando] = useState(true);
  const [erro, setErro] = useState<string | null>(null);

  const { colunas, widths, startResize, reordenar, totalWidth } = useResizableColumns(
    `@SolucaoTS:grid:colaboradores:v1:${logado?.id ?? 'anon'}`,
    COLUNAS,
  );
  const buscaDebounced = useDebounce(busca, 400);

  const filtros = useMemo<FiltrosColaborador>(() => {
    const f: FiltrosColaborador = {};
  ...
  useEffect(() => {
    ...
    listarColaboradores({
      ...
      .catch((e) => {
        if (!ativo) return;
        setErro(extrairMensagemErro(e, 'Não foi possível carregar os colaboradores.'));
        setResultado(vazio);
      })
  ...
  function renderCell(key: string, c: Colaborador): ReactNode {
    switch (key) {
      case 'acoes':
        return (
          <AcoesGrade recurso="CADASTRO_COLABORADORES">
      ...
      case 'colaborador':
        return (
          <div className="min-w-0">
            <p className="truncate font-medium text-ink-800 dark:text-ink-100" title={c.nome}>{c.nome}</p>
            <p className="truncate text-xs text-ink-400" title={c.cargo}>{c.cargo}</p>
          </div>
        );
      case 'cpf':
        return <span className="block truncate font-mono text-xs text-ink-600 dark:text-ink-300">{revelado ? mascaraCpfCnpj(c.cpf) : mascararDocumento(c.cpf)}</span>;
      case 'admissao':
        return <span className="block truncate text-ink-600 dark:text-ink-300">{dataBr(c.dataAdmissao)}</span>;
      case 'salario':
        return <span className="block truncate text-ink-600 dark:text-ink-300">{formatarMoeda(c.salarioContratual)}</span>;
      default:
        return null;
    }
  }
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-ink-200/70 bg-white shadow-card dark:border-ink-800/70 dark:bg-ink-900">
      ...
      <div className="flex flex-col items-center justify-between gap-3 border-t border-ink-100 px-4 py-3 text-xs text-ink-400 dark:border-ink-800 sm:flex-row">
        <span>{total > 0 ? `${total} colaborador(es)` : 'Sem registros'}</span>
