...
const COLUNAS: ColunaDef[] = [
  ...
  { key: 'numero', label: 'Número', width: 180, sortKey: 'numeroEmpenho' },
  { key: 'ano', label: 'Ano', width: 100, sortKey: 'anoEmpenho' },
  { key: 'emissao', label: 'Emissão', width: 140, sortKey: 'dataEmissaoEmpenho' },
  { key: 'retificacao', label: 'Retificação', width: 130, sortKey: 'retificacao' },
];
...
export function EmpenhosTab({ ajusteId }: { ajusteId: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <GradeSimples
        storageKey="@SolucaoTS:grid:empenhosAjuste:v1"
        colunas={COLUNAS}
        dados={lista}
        chave={(e) => e.id}
        ...
        onDuploClique={(e) => setModal({ tipo: 'form', empenho: e })}
        valorOrdenacao={(campo, e) => {
          if (campo === 'numeroEmpenho') return e.numeroEmpenho;
          if (campo === 'anoEmpenho') return e.anoEmpenho;
          if (campo === 'dataEmissaoEmpenho') return e.dataEmissaoEmpenho;
          if (campo === 'retificacao') return e.retificacao ? 'Sim' : 'Não';
          return null;
        }}
        renderCell={(coluna, e) => {
          switch (coluna) {
            ...
            case 'numero':
              return <span className="block truncate font-mono text-xs text-ink-800 dark:text-ink-100">{e.numeroEmpenho}</span>;
            case 'ano':
              return <span className="block truncate text-ink-600 dark:text-ink-300">{e.anoEmpenho}</span>;
            case 'emissao':
              return <span className="block truncate text-ink-600 dark:text-ink-300">{dataBr(e.dataEmissaoEmpenho)}</span>;
            case 'retificacao':
              return e.retificacao ? <Badge tone="warning">Sim</Badge> : <Badge tone="neutral">Não</Badge>;
            default:
              return null;
          }
        }}
