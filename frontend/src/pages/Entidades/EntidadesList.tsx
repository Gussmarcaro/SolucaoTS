...
interface Props {
  refreshKey: number;
  onVisualizar: (entidade: Entidade) => void;
  onEditar: (entidade: Entidade) => void;
  onAlternarStatus: (entidade: Entidade) => void;
}
...
export function EntidadesList({ refreshKey, onVisualizar, onEditar, onAlternarStatus }: Props) {
  ...
  const [resultado, setResultado] = useState<Paginado<Entidade>>(vazio);
  const [carregando, setCarregando] = useState(true);
  const [erro, setErro] = useState<string | null>(null);

  const { colunas, widths, startResize, reordenar, totalWidth } = useResizableColumns(
    `@SolucaoTS:grid:entidades:v1:${logado?.id ?? 'anon'}`,
    COLUNAS,
  );
  const buscaDebounced = useDebounce(busca, 400);

  const filtros = useMemo<FiltrosEntidade>(() => {
    const f: FiltrosEntidade = {};
  ...
  useEffect(() => {
    ...
    listarEntidades({
      ...
      .catch((e) => {
        if (!ativo) return;
        setErro(extrairMensagemErro(e, 'Não foi possível carregar as entidades.'));
        setResultado(vazio);
      })
  ...
  function renderCell(key: string, ent: Entidade): ReactNode {
    switch (key) {
      case 'acoes':
        return (
          <AcoesGrade recurso="CADASTRO_ENTIDADES">
            ...
            <IconBtn exige="EDICAO" title="Editar" onClick={() => onEditar(ent)}>
              <Pencil className="h-4 w-4" />
            </IconBtn>
            <IconBtn exige="EDICAO" title={ent.ativo ? 'Inativar' : 'Reativar'} danger={ent.ativo} onClick={() => onAlternarStatus(ent)}>
              <Power className="h-4 w-4" />
            </IconBtn>
          </AcoesGrade>
        );
      case 'status':
        return <Badge tone={ent.ativo ? 'success' : 'neutral'}>{ent.ativo ? 'Ativo' : 'Inativo'}</Badge>;
      case 'entidade':
        return (
          <div className="min-w-0">
            <p className="truncate font-medium text-ink-800 dark:text-ink-100" title={ent.razaoSocial}>{ent.razaoSocial}</p>
            {ent.nomeFantasia && <p className="truncate text-xs text-ink-400">{ent.nomeFantasia}</p>}
          </div>
        );
      case 'cnpj':
        return <span className="block truncate font-mono text-xs text-ink-600 dark:text-ink-300">{mascaraCpfCnpj(ent.cnpj)}</span>;
      case 'cidadeuf':
        return <span className="block truncate text-ink-600 dark:text-ink-300">{ent.cidade} / {ent.uf}</span>;
      default:
        return null;
    }
  }
        ...
        <table className="w-full text-left text-[13px]" style={{ tableLayout: 'fixed', minWidth: totalWidth }}>
          ...
          <tbody className="divide-y divide-ink-100 dark:divide-ink-800">
            {carregando ? (
              ...
              <tr>
                <td colSpan={colunas.length} className="py-16 text-center">
                  <p className="text-sm text-ink-500 dark:text-ink-400">Nenhuma entidade encontrada.</p>
                </td>
              </tr>
            ) : (
              data.map((ent) => (
