...
- **Cartão da Agenda**, logo abaixo da execução — os próximos 7 dias e o que ficou sem registro. Compromisso é a única coisa do sistema com **hora marcada**: prazo se cumpre até o fim do dia, reunião começa às 14h30. É o segundo painel da tela, antes das contagens, pelo mesmo motivo da faixa de execução: "quantos fornecedores tenho" não muda o que se faz hoje. Reusa `GET /compromissos` com a mesma janela e a **mesma visibilidade** da tela da Agenda — consulta própria seria a chance de o Dashboard mostrar o particular que a Agenda esconde.
...
- Cor é **token da paleta**, não hex: a tela decide como pintar e o tema escuro continua legível. A mesma cor aparece de duas formas — **cheia** na bolinha e nos blocos da grade horária, e em **marca d'água** no fundo da linha (mês e lista). A bolinha identifica; a lavagem faz o olho *agrupar* sem comparar pontos de 6px. Fica a 10% no claro e 20% no escuro, porque sobre fundo escuro a mesma opacidade quase some; **cancelado fica de fora**, senão a linha riscada teria o mesmo peso visual das que ainda valem. As quatro variantes de cada cor estão escritas **por extenso** em `CORES` — o Tailwind só gera o que consegue ler no código, e classe concatenada em tempo de execução passa no typecheck e não pinta nada. `npm test` trava essa forma literal.
...
- O mesmo script confere **o menu do frontend**: tela em `navigation.ts` sem `recurso` não aparece na matriz, e o administrador configura tudo que vê concluindo que cobriu o sistema. Exceções se justificam em `MENU_SEM_RECURSO` — hoje só o Dashboard (porta de entrada; cada painel dentro dela confere a própria permissão) e o placeholder `/execucao/tecnico`. As telas de Execução que ganharam backend (Despesas, Receitas, Pagamentos, Guias, Conciliação, Contas Bancárias) já declaram recurso, e o Rateio saiu de lá para `Cadastro → Financeiro`.
- **Duas conferências sobre a própria conferência**, ambas nascidas de buracos reais:
  - **Exceção que sobrevive à tela é reprovada.** A entrada de `/execucao/financeiro/rateio` ficou na lista depois que a tela mudou de lugar. Ruído hoje, armadilha amanhã: o dia em que alguém criar uma rota naquele caminho, ela nasce fora da matriz com uma justificativa plausível já escrita.
  - **O leitor de `navigation.ts` confere a própria cobertura.** Ele casava `{ label: '…' … to: '…' }` numa tacada, o que exigia `{ label:` na mesma linha — item quebrado em várias linhas (o que o formatador faz com rótulo longo) **não existia para o script**, e a checagem dizia "tudo ok" sobre uma tela que nunca viu. Agora a busca é ancorada em `to:` e o total lido é comparado com o total de rotas do arquivo.
